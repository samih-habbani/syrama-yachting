// The full, unfiltered charter fleet across every destination — was the
// charter side of /yachting/fleet (which now permanently redirects here,
// see next.config.ts). Lives under /yacht-charter so every charter URL on
// the site shares one prefix, alongside the destination picker
// (/yacht-charter) and each region/city's own page.
//
...
const TITLE = 'All Yachts for Charter'
const DESCRIPTION = 'Browse our complete fleet of luxury yachts available for charter across the French Riviera, Balearic Islands, Greece, Dubai and more. Filter by destination, budget, guests and length.'

export const metadata: Metadata = {
  title: TITLE,
  description: DESCRIPTION,
  alternates: { canonical: '/yacht-charter/all-yachts' },
  ...
  openGraph: {
    ...
    description: DESCRIPTION,
    url: `${SITE_URL}/yacht-charter/all-yachts`,
...
export default async function AllCharterYachtsPage() {
  ...
  const initialYachts = await getYachts({ type: 'charter', limit: 500 })
    ...
    <div className="flex flex-col min-h-screen bg-[#06090f]">
      ...
      <main id="main-content" className="flex-1 pt-20">
        <FleetWrapper
          ...
          seo={{
            eyebrow: 'Full Fleet',
            h1: 'All Yachts Available for Charter',
            intro: ['Browse our complete charter fleet across every destination we serve — filter by destination, length, guests or budget, or pick a specific region to see its own dedicated page.'],
            name: 'All Yachts',
          }}
        />
        <Contact />
