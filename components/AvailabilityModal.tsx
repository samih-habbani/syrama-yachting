...
export default function AvailabilityModal({ isOpen, onClose, yacht }: AvailabilityModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const lines = [
      `Hello Syrama Yachting! I'd like to check availability for the *${yacht.model}*${yacht.builder ? ` (${yacht.builder})` : ''}${yacht.length ? `, ${yacht.length}m` : ''}.`,
      '',
      `📍 City: ${city}`,
      `📅 Date: ${date}`,
      `👥 Guests: ${guests}`,
  ...
  return (
    ...
    >
      <style>{`
        ...
        @keyframes availSlideUp {
          from { opacity: 0; transform: translateY(20px) scale(0.98); }
          to { opacity: 1; transform: translateY(0) scale(1); }
        }
        .availability-modal-content {
          animation: availSlideUp 0.35s cubic-bezier(0.25, 0.1, 0, 1);
        }
      `}</style>
